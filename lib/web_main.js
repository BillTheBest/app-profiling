						...
						fs.readFile(filePath, 'utf8', (function(device_name, report_id){ return function (err, str) {
							...
							for(var i=1; i<data.length; i++){
					        	...
					        	if(data[i].method == "Network.webSocketFrameReceived"){
					        		...
					        		reports[device_name][report_id].websocketframe_received_bytes += JSON.stringify(data[i].params.response.payloadData).length;
					        	...
					        	if(data[i].method == "Network.webSocketFrameSent"){
					        		...
					        		reports[device_name][report_id].websocketframe_sent_bytes += JSON.stringify(data[i].params.response.payloadData).length;
					        	}
...
app.get('/functionstime/:devname', function(req, res){
    ...
    for(var j=0; j<sorted_keys.length; j++){
    	...
    	for(var i in reports[device_name][report_id].functions){
    		if(!functions_data[i])
    			functions_data[i] = [];
    		functions_data[i].push(reports[device_name][report_id].functions[i].time);
    	}
        executions_data.push(executions_description[report_id]);
    }
