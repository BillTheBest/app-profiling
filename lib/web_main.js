...
app.get('/heap/', function(req, res){
    ...
    var data = {data: heap_data, max:max_heap_data};
...
app.get('/functionstime/', function(req, res){
    ...
    var foo = ["test 1", "test 2", "test 3"];
    var data = {data:functions_data, executions:Object.keys(reports)};



    // 	functions_data[report_id] = [];
    // 	for(var i in reports[report_id].functions){
    //     	if(reports[report_id].functions[i].time != 0){
    //         	functions_data[report_id].push(reports[report_id].functions[i].time);
    //         	if(firstIndex)
    //         		categories_data.push(i);
    //         }
    //     }
    //     firstIndex = false;
    // }
    
    //var data = {data:functions_data, categories:categories_data};
    //var data = functions_data;
...
//     	for(var i in reports[report_id].functions)
//         	if(reports[report_id].functions[i].time != 0)
//             	functions_data[report_id].push([i, reports[report_id].functions[i].time]);
//     }
    
//     var body = JSON.stringify(functions_data);
//     res.setHeader('Content-Type', 'text/plain');
//     res.setHeader('Content-Length', body.length);
//     res.end(body);
// });
...
app.get('/rpcfrequency/', function(req, res){
    ...
    var data = {categories:["Sent", "Received"], data:frequency_data};
...
app.get('/rpctraffic/', function(req, res){
    ...
    var data = {categories:["Sent", "Received"], data:traffic_data};
    var body = JSON.stringify(data);
    res.setHeader('Content-Type', 'text/plain');
    res.setHeader('Content-Length', body.length);
    res.end(body);
