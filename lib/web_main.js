...
var websocketframe_received_count = 0;
var websocketframe_sent_count = 0;
var websocketframe_received_bytes = 0;
var websocketframe_sent_bytes = 0;
	...
	fs.readFile(filePath, function(err,str) {
	        ...
	        for(var i=1; i<data.length; i++){
	        	if(data[i].method == "Network.webSocketFrameReceived"){
	        		websocketframe_received_count++;
	        		websocketframe_received_bytes += JSON.stringify(data[i].params.response.payloadData).length;
	        	...
	        	if(data[i].method == "Network.webSocketFrameSent"){
	        		websocketframe_sent_count++;
	        		websocketframe_sent_bytes += JSON.stringify(data[i].params.response.payloadData).length;
	        		//console.log("sent: %s, payload: %s", sent_count++, JSON.stringify(data[i].params.response.payloadData).length);
	        	}
	        ...
	        console.log("webSocketFrameSent : #%s, bytes: %s" , websocketframe_sent_count, websocketframe_sent_bytes);
	        console.log("webSocketFrameReceived : #%s, bytes: %s" , websocketframe_received_count, websocketframe_received_bytes);
...
app.get('/rpcfrequency/', function(req, res){
    ...
    var data = {categories:["Sent", "Received"], data:[websocketframe_sent_count, websocketframe_received_count]};
    var body = JSON.stringify(data);
    res.setHeader('Content-Type', 'text/plain');
    res.setHeader('Content-Length', body.length);
    res.end(body);
...
app.get('/rpctraffic/', function(req, res){
    ...
    var data = {categories:["Sent", "Received"], data:[websocketframe_sent_bytes, websocketframe_received_bytes]};
    var body = JSON.stringify(data);
    res.setHeader('Content-Type', 'text/plain');
    res.setHeader('Content-Length', body.length);
    res.end(body);
