...
var runner = function(suite,caseNo){
    ...
    var outFile = join(device.options.path,device.options.suitefile.replace("json","")+caseNo+"."+btoa(device.device)+"."+Date.now()+".raw"),fileLocked=false;

    fs.appendFileSync(outFile, '[');
    fs.appendFileSync(outFile, JSON.stringify({"suite":suite,"description":device.options.desc})+",");
